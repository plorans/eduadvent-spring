...
public interface CicloDao extends JpaRepository<Ciclo, Long> {
    ...
    @Query(value = "SELECT COALESCE(MAX(SUBSTR(CICLO_GRUPO_ID,1,8)),'0') " +
            "AS CICLO_ID FROM CICLO_GRUPO_CURSO WHERE EMPLEADO_ID = :cicloId", nativeQuery = true)
    String getMejorCarga(String cicloId);

    @Query(value = "SELECT CICLO_ID FROM CICLO WHERE NOW() BETWEEN F_INICIAL AND F_FINAL", nativeQuery = true)
    String getMejorCargaNow();
