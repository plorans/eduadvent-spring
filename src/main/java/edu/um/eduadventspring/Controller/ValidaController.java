...
public class ValidaController {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!codigoId.equals("x") && estadoEscuela.equals("A") && activo) {
			...
			Collections.sort(menu, new Comparator<Modulo>() {
				@Override
				public int compare(Modulo modulo1, Modulo modulo2) {
					return modulo1.getNombreModulo().compareTo(modulo2.getNombreModulo());
				}
			});

			Collections.sort(opcion, new Comparator<ModuloOpcion>() {
				@Override
				public int compare(ModuloOpcion mo1, ModuloOpcion mo2) {
					return mo1.getNombreOpcion().compareTo(mo2.getNombreOpcion());
				}
			});
