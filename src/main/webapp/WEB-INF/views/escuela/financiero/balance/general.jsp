...
<%
	...
	for(int i = 0; i < contMascara.getMascBalance().length(); i++){
		if(i >= nivel){//Esto es para obtener los 0's de la wildcard
			for(int j = 0; j < Integer.parseInt(contMascara.getMascBalance().substring(i,i+1)); j++){
				wildcard += "0";
			}
		}else{//Esto es para obtener el tamaño de la mascara (lo anterior a los 0's)
			mascaraLength += Integer.parseInt(contMascara.getMascBalance().substring(i,i+1));
		}
	}
	
	lisMayor	= contMayorL.getListBalance(conElias, ejercicioId, String.valueOf(mascaraLength), wildcard, "ORDER BY MAYOR_ID");
...
<table align="center" width="95%">
	...
	<tr>
		<td class="titulo">Balance General</td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
