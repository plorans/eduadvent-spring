...
<head>
	<script>	
		...
		function SubirCodigo( CodigoPersonal ){
			alert("SubirCodigo:"+CodigoPersonal);
...
<%
	...
	switch (nAccion){
		case 1:{
			...
			lisAlumno = alumnoLista.getArrayList(conElias, escuelaId, strNombre, strPaterno, strMaterno,"ORDER BY 4,5,3");			
			if (lisAlumno.size() > 0)
				strResultado	= "Click sobre el nombre del alumno";
			else
		...
		case 2:{
			...
			if (request.getParameter("CodigoPersonal").substring(0,3).equals(escuelaId.length()==1?("0"+escuelaId):escuelaId) && alumno.existeReg(conElias)==true){
				alumno.mapeaRegId(conElias, request.getParameter("CodigoPersonal"));
				strResultado = "Click sobre el nombre del alumno";
			}else{
				strResultado = "No existe: "+request.getParameter("CodigoPersonal");
			}
			break;
		}
		case 3:{
  ...
  <tr align='CENTER'>
    <td>Nom.  
		...
		Mat.
			...
			<a href="javascript:BuscarNombre()" class="btn">Buscar</a>
	</td>
  </tr>
</form>
  <tr>
...
<table width="77%" border="0" align="center" class="table table-condensed table-striped">
<tr>
...
<tr>
  ...
  <th width="25%" align="center">Nivel</th>
  <th width="10%" align="center">Grado</th>
...
%>				
  <tr >
    ...
    <td>
	  <a href="javascript:SubirCodigo('<%=alumno.getCodigoId()%>')">  
	  	<%=alumno.getApaterno()%>&nbsp;<%=alumno.getAmaterno()%>;&nbsp;<%=alumno.getNombre()%>
	  </a>
	</td>
...
<%				if(lisAlumno.size()==1){
					alumno = (aca.alumno.AlumPersonal) lisAlumno.get(0);					
					session.setAttribute("codigoAlumno", alumno.getCodigoId());
					session.setAttribute("codigoReciente", alumno.getCodigoId());
...
%>		
  <tr> 
    ...
    <td>
	  <a href="javascript:SubirCodigo('<%=alumno.getCodigoId()%>')">
	  	<%=alumno.getNombre()%>&nbsp;<%=alumno.getApaterno()%>&nbsp;<%=alumno.getAmaterno()%>
	  </a>
	</td>
  </tr>	
<%  		
