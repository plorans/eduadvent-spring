...
<%
	...
	Vector vecTipo 		= contTipoL.getVecAll(conElias, "ORDER BY TIPO_ID");
	Vector vecAuxiliar	= null;
	Vector<CatEscuela> vecEscuelas	= null;
...
<%
		vecEscuelas = catEscuelaL.getVecAll(conElias, "ORDER BY ESCUELA_NOMBRE");
		for(int i = 0; i < vecEscuelas.size(); i++){
			catEscuela = (CatEscuela) vecEscuelas.get(i);
%>
...
<%
	for(int i = 0; i < vecTipo.size(); i++){
		contTipo = (ContTipo) vecTipo.get(i);
%>
...
<%
	vecAuxiliar	= contAuxiliarL.getVecTipo(conElias, tipoCta, escuelaId, "ORDER BY AUXILIAR_NOMBRE");
	for(int i = 0; i < vecAuxiliar.size(); i++){
		contAuxiliar = (ContAuxiliar) vecAuxiliar.get(i);
%>
			<tr>
				<td>
