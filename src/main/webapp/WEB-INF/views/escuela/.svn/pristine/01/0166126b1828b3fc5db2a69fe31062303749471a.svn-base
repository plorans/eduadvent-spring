...
<div id="content">
	...
	if(request.getParameter("generar")!=null){
		FinAlumSaldos fnSaldo = new FinAlumSaldos();
		
		List<FinEdoCtaReporte> lsSaldos = new ArrayList();
		lsSaldos.addAll(fnSaldo.saldoAlumno(conElias, request));
		System.out.println(lsSaldos.size());
		List<String> agrupador = new ArrayList();
		for(FinEdoCtaReporte s : lsSaldos){
			if(!agrupador.contains(s.getNivelgradogrupo())){
				agrupador.add(s.getNivelgradogrupo());
			}
		}
	...
	<table class="table table-bordered" style="width: 80%; margin: 0px auto;" >
		...
		%>
			...
			BigDecimal totAc = BigDecimal.ZERO;
			BigDecimal totDe = BigDecimal.ZERO;
			
			for(FinEdoCtaReporte s : lsSaldos){
				if(s.getNivelgradogrupo().equals(ag)){
					if(s.getSaldo().compareTo(BigDecimal.ZERO)!=0){
					String acreedor = "";
					String deudor = "";
					if(s.getSaldo().compareTo(BigDecimal.ZERO)<0){
						acreedor = s.getSaldo().abs().toString();
						totAc = totAc.add(s.getSaldo().abs());
					}else if(s.getSaldo().compareTo(BigDecimal.ZERO)>0){
						deudor = s.getSaldo().toString();
						totDe = totDe.add(s.getSaldo());
					}
			%>
			<tr>
				<td><%= s.getCodigoid() %></td>
				<td><%= s.getNombre() %></td>
				<td style="text-align: right;"><%= deudor %></td>
				<td style="text-align: right;"><%= acreedor %></td>
			</tr>
	    ...
	    %>
			<tr>
				<td></td>
				<td></td>
				<td style="text-align: right;"><%= totDe %></td>
				<td style="text-align: right;"><%= totAc %></td>
			</tr>
			<tr>
				<td colspan="4" style="border-right: 0px; border-left: 0px;">&nbsp</td>
			</tr>
			<%
		}
		%>
...
<script>
	...
	$('#cicloid').val('<%= request.getParameter("ciclo_id")!=null ? request.getParameter("ciclo_id") : "" %>').change(); 
