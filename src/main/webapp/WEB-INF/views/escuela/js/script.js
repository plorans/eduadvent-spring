...
TINY.table=function(){
	...
	function cp(f,c){
		...
		//si es fecha (si f y c al hacerle split("/") pone en un arreglo 3 objetos es FECHA)
		var splitf = f.split ("/");
		var splitc = c.split("/");
		if(splitf.length == 3 && splitc.length ==3){
			i=Date.parse(fX); n=Date.parse(fY);
		}
		
		//si es fecha (si f y c tiene '/' es fecha)
		/*if(f.indexOf("/") != -1 && c.indexOf("/") != -1){
			i=Date.parse(fX); n=Date.parse(fY);
		}*/
		if(!isNaN(i)&&!isNaN(n)){g=i; h=n}
		return g>h?1:(g<h?-1:0)
